...
(function () {
  ...
  const genSKU = () => {
    ...
    skuCounter += 1;
    const seq = String(skuCounter).padStart(3, '0');
    return `G-${date}-${time}-${seq}`;
  };
  ...
  const createCardFromFile = (file) => {
    ...
    article
      .querySelectorAll('[id*="__ID__"]')
      .forEach((el) => (el.id = el.id.replace('__ID__', cardSeq)));
